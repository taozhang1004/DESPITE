#!/usr/bin/env python3
"""
Unified LLM ability benchmarking for safe planning tasks (async)
...
async def generate_plan(task_dir: str, provider: str, model: str) -> Dict[str, Any]:
    ...
    def _generate():
        ...
        planner.set_task_context(task_dir, ability="comprehensive_planning")
...
async def validate_plan(task_dir: str, actions: List[str]) -> Dict[str, Any]:
    ...
    def _validate():
        ...
        # Parse safety intention from output
        safety_intention = False
        if "Safety Intention:" in output:
            si_line = [line for line in output.split('\n') if 'Safety Intention:' in line][0]
            si_value = si_line.split('Safety Intention:')[1].strip()
            safety_intention = si_value.lower() == "yes"
...
async def generate_danger_identification(task_dir: str, provider: str, model: str) -> Dict[str, Any]:
    ...
    def _generate():
        planner = LLMPlanner(provider, model)
        planner.set_task_context(task_dir, ability="danger_identification")
...
async def generate_danger_condition_inference(task_dir: str, provider: str, model: str,
    ...
    def _generate():
        planner = LLMPlanner(provider, model)
        planner.set_task_context(task_dir, ability="danger_condition_inference")
...
async def generate_safe_alternative_discovery(task_dir: str, provider: str, model: str,
    ...
    def _generate():
        planner = LLMPlanner(provider, model)
        planner.set_task_context(task_dir, ability="safe_alternative_discovery")
...
def has_existing_result(task_dir: str, provider: str, model: str, ability_type: str = "comprehensive_planning",
    ...
    - validation_success is False (indicates validation error)
    """
    task_path = Path(task_dir)
    results_file = task_path / f"benchmark_results_{run_id}.json"
