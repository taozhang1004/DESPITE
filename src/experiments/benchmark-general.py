...
from src.utils.logic_utils import compare_logic_expressions
...
async def generate_danger_condition_inference(task_dir: str, provider: str, model: str, 
    ...
    def _generate():
        ...
        # Clean response using deepseek to extract only JSON
...
def validate_danger_condition_inference(llm_conditions: Dict[str, str], 
    ...
    for action, gt_condition in gt_dangerous_actions.items():
        ...
        if predicted_condition:
            ...
            try:
                is_match = compare_logic_expressions(predicted_condition, gt_condition_str)
                results[action] = {
                    "correct": is_match,
    ...
    try:
        if ability_type == "comprehensive_planning":
            ...
            validation = validate_danger_condition_inference(
                condition_result.get("inferred_conditions", {}), gt_dangerous_actions
            )
...
if __name__ == "__main__":
    ...
    MAX_CONCURRENT = 8   # Number of parallel runs
